...
def load_data(batch_size):
    ...
    """
    base_dir = "/app/dev/jungmin/2class_noisy_vs_nonnoisy/cnn_only_v2/outputs"
    X = np.load(os.path.join(base_dir, "X_cnn.npy"))  # [B, 86, 13]
    y = np.load(os.path.join(base_dir, "y_cnn.npy"))

    # stratified 분할
    (X_train, y_train), (X_val, y_val), (X_test, y_test) = stratified_split(X, y)

    # 텐서 변환 (CNN을 위한 채널 차원 추가, 라벨은 long tensor)
    def to_tensor(X, y):
        X_tensor = torch.tensor(X, dtype=torch.float32).unsqueeze(1)  # [B, 1, 86, 13]
        y_tensor = torch.tensor(y, dtype=torch.long)  # CrossEntropyLoss → long
